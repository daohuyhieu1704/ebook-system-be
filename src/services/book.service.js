...
class BookService {
    ...
    async CreateBook({ book }) {
        try {
            let { title, description, image, price, author_ID, category_ID, inventory_ID } = book;
            let my_book = await Book.create({
                title, description, image, price, author_ID, category_ID, inventory_ID
            })  
    ...
    async UpdateBook({ id, book }) {
        try {
            ...
            await Book.update({...book, modified_at: Date.now()}, {
                where: { id}
            })
